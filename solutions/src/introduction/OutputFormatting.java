...
public class OutputFormatting {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < 3; i++){
            String s1 = sc.next();
            int x = sc.nextInt();
            int padding = 15 - s1.length();
            String firstCol = s1 + " ".repeat(padding);
            String strX = Integer.toString(x);
            int paddingX = 3 - strX.length();
            String secondCol = "0".repeat(paddingX) + strX;
            System.out.printf("%s%s%n", firstCol, secondCol);
        }
        System.out.println("================================");
        sc.close();
    }
